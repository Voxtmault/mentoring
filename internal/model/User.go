...
func AddUser(username, password, role string) (class.Response, error) {
    var obj class.User
    ...
    err = con.QueryRow(query, username).Scan(&obj.ID)
    ...
    _, err = con.Exec(insertQuery, username, hashedPassword, role)
    if err != nil {
...
func UpdateUser(id int, username, role string) (class.Response, error) {
    con, err := db.DbConnection()
    if err != nil {
    ...
    stmt, err := con.Prepare("UPDATE users SET username=?, role=? WHERE id=?")
    if err != nil {
    ...
    _, err = stmt.Exec(username, role, id)
    if err != nil {
